...
export default {
    ...
    data () {
        return {
            portA: ports.a,
        // 表格内容
            columns14: [{title: '区域', align: 'center', key: 'area'},{title: '省份', align: 'center', key: 'province'},{title: '家化业务员', align: 'center', key: 'salesman'},{title: '原始得分', align: 'center', key: 'originalscore'},{title: '标准得分', align: 'center', key: 'standardscore'},
    ...
    mounted(){
    // 请求列表总数据
        let url = this.portA + this.constData.RM_FR_SPR;
        ...
        this.httpClient.get(url,params).then((res)=>{        
            ...
            if (this.pageData.length < 10) {
                this.data14 = this.pageData
            } else {
                for(let i = 0; i<10; i++){
                    this.data14.push(this.pageData[i])
                }
            }
        });
    },
    ...
    //导出表格数据
        exportData(){
            window.location.href= this.portA+this.constData.RM_FR_DR_Export+"?hpid="+this.$route.query.id+"&id=3"
        },
    // 换页操作
        changPage(page){
            this.data14 = []
...
.salesPersonReport{min-width: 800px;
